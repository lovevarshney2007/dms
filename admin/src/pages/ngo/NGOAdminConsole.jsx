    ...
    <>
      ...
      >
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 border-none text-left ${
                  isActive
                    ? "bg-coral text-cream shadow-lg shadow-coral/20"
                    : "bg-transparent text-cream/60 hover:bg-cream/5 hover:text-cream"
                }`}
              >
        ...
        <div className="p-4 border-t border-cream/10 space-y-1.5">
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-coral hover:bg-coral/10 transition-all duration-200 bg-transparent border-none text-left cursor-pointer"
          >
            ...
            {[
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-2.5 px-4 py-3 rounded-xl border border-charcoal/10 bg-transparent text-left hover:border-coral/40 hover:bg-coral/5 transition-all duration-200 text-sm font-medium text-charcoal"
                >
...
function VolunteersManager({ showToast }) {
  ...
  const load = () => {
    setLoading(true);
    getVolunteers().then(res => setVolunteers(res.items || res || [])).catch(() => showToast("Failed to load volunteers")).finally(() => setLoading(false));
  };
  useEffect(load, []);
        ...
        <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-charcoal/5 px-5 md:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              ...
              className="md:hidden text-charcoal p-1.5 -ml-1.5 bg-transparent border-none"
            >
