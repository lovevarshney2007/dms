...
import { Plus, Edit2, Trash2, Layers, X, Check, RefreshCw } from 'lucide-react';
import { api } from '../../lib/api';
import { showToast } from '../../components/Toast';
import EmptyState from '../../components/EmptyState';
import { resolveImg } from '../../config';

const EMPTY = { title: '', subtitle: '', description: '', imageUrl: '', link: '' };
...
export default function InitiativesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.get('/admin/content/ngo-initiative')
  ...
  const save = async e => {
    ...
    try {
      if (editing) {
        ...
        const created = await api.post('/admin/content/ngo-initiative', form);
        setRows(r => [created, ...r]);
    ...
    <div className="page">
      <div className="page-header">
        <div className="flex-between">
          <div>
            <h1 className="page-title">Initiatives Content</h1>
            <p className="page-subtitle">Manage NGO initiatives and programs</p>
          ...
          <div className="flex gap-2">
            ...
            <button className="btn btn-green btn-sm" onClick={openAdd}><Plus size={13} />Add Initiative</button>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        {loading ? (
          ...
          <EmptyState icon={Layers} title="No initiatives yet" desc="Add NGO initiatives using the button above" />
        ) : (
            ...
            {rows.map(r => (
              <div key={r._id} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 16 }}>
                  <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: 15, marginBottom: 4 }}>{r.title}</div>
                  {r.subtitle && <div style={{ fontSize: 12, color: 'var(--green)', marginBottom: 6 }}>{r.subtitle}</div>}
                  {r.description && <div style={{ fontSize: 12, color: 'var(--text-soft)', lineHeight: 1.5, marginBottom: 10 }}>{r.description.substring(0, 80)}...</div>}
          ...
          <>
            ...
            <motion.div className="detail-drawer" style={{ width: 440 }} initial={{ x: 460 }} animate={{ x: 0 }} exit={{ x: 460 }} transition={{ duration: 0.28 }}>
              <div className="detail-drawer-head">
                <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: 16 }}>{editing ? 'Edit' : 'Add'} Initiative</div>
              ...
              <form onSubmit={save}>
                <div className="detail-drawer-body" style={{ gap: 14 }}>
                  {[
                    ['title', 'Title', 'text'],
                    ['subtitle', 'Subtitle / Tagline', 'text'],
                    ['imageUrl', 'Image URL', 'url'],
                    ['link', 'Learn More Link (optional)', 'url'],
                  ].map(([name, label, type]) => (
